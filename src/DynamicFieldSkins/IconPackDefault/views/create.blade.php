...
<div class="my{{$config->get('id')}}_div {{$data['column']}} xi-3x" onclick="icon_off(this,'{{$config->get('id')}}')"></div><br>
...
<div class="{{$config->get('id')}}_layer">
    ...
    <div class="layer_in">
        ...
        @foreach($icon_action as $value)
            <div class="{{$value}} xi-2x" onclick="icon_on(this, '{{$config->get('id')}}')"></div>
        @endforeach
        ...
        @foreach($icon_spinner as $value)
            <div class="{{$value}} xi-2x xi-spin" onclick="icon_on(this, '{{$config->get('id')}}')"></div>
        @endforeach
...
<script>
    ...
    function icon_on(my_div, config_get_id) {
        if(my_div.getAttribute('class').includes("xi-spin")){
            document.querySelector(".my"+config_get_id+"_div").setAttribute('class',"my"+config_get_id+"_div xi-3x xi-spin");
        }else{
            document.querySelector(".my"+config_get_id+"_div").setAttribute('class',"my"+config_get_id+"_div xi-3x");
        }

        var my_iconClass = my_div.getAttribute('class').replace(" xi-2x","").replace(" xi-spin", "");
