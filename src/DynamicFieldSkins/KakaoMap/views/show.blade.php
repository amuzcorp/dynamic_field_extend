...
<div class="xe-form-group xe-dynamicField">
    ...
    <div class="map_wrap"
         style="@if($args[$config->get('id')."_list_display"] == "true")width:70%@else width:100% @endif;height:410px;float:left;position: relative;@if(json_decode($args[$config->get('id')."_location_data"]) == null) display: none; @elseif(!array_filter(json_decode($args[$config->get('id')."_location_data"]))) display: none; @endif">
        <div class="map" id="{{$config->get('id')}}_map" style="width:100%;height:400px;float:left;"></div>
        <!-- 지도타입 컨트롤 div 입니다 -->
    ...
    <div class="store-list" id="{{$config->get('id')}}_store_list"
         style="width:28%;height:400px;overflow:auto;@if($args[$config->get('id')."_list_display"] != "true") display:none;@endif">
        <div class="row_map">
            @if(!empty(json_decode($args[$config->get('id')."_location_data"])))
